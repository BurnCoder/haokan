...
//			},500);
	...
	$('.ns_list_hot .ns_list_hot_but').on('click',function(){
		...
		for(var i=0; i<oHotBook[nHotnum].length; i++){
			shot+='<li><a href="/index/book/info?book_id='+ oHotBook[nHotnum][i]['book_id'] +'">'+oHotBook[nHotnum][i]['name']+'</a></li>';
		}
...
//			}
			...
			if(data.length>0){
				...
				for(var i=0; i<data.length; i++){
					if(!oHotBook[parseInt(i/10)]){
						oHotBook.push([data[i]]);
					}else{
						oHotBook[parseInt(i/10)].push(data[i]);
					}
					if(i<10){
						shot+='<li><a href="/index/book/info?book_id='+ data[i]['book_id'] +'">'+data[i]['name']+'</a></li>';
					}
				}
...
function searchtips(keyword,boolean){
	if(searchstarted){
		...
		$.ajax({
			url:'/index/index/searchapi?&keyword='+keyword+'&page='+pageIndex,
			type:'get',
			timeout: ntimeout,
			cache: false,
			success: function(data){
